...
const run = async (interaction: CommandInteraction) => {
	...
	if (
		cooldownData.cooldowns.some((cooldown) => cooldown.type === "crime" && cooldown.endTime >= new Date())
	) {
	...
	try {
		...
		if (componentInteraction.componentType === ComponentType.SelectMenu) {
			...
			updateBalance(balanceData, crimePay);
			addCooldown(cooldownData, "crime", endTime, channel?.id ?? user.id);
